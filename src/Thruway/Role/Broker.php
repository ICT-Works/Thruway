...
use IctWorks\SharedKernel\Application\CQRS\WriteModel\CommandBus;
use JuanPescador\Infrastructure\WebSocket\Thruway\SubscriptionGroup;
...
class Broker implements RealmModuleInterface
{
    ...
    /**
     * @var \IctWorks\SharedKernel\Application\CQRS\WriteModel\CommandBus
     */
    private $commandBus;

    /**
     * @param \IctWorks\SharedKernel\Application\CQRS\WriteModel\CommandBus $commandBus
     */
    ...
    protected function processSubscribe(Session $session, SubscribeMessage $msg)
    {
        ...
        if (!isset($this->subscriptionGroups[$matchHash])) {
            ...
            $this->subscriptionGroups[$matchHash]->setCommandBus($this->commandBus);
        }
